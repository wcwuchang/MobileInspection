 ...
 * 新建专项任务
 */
public class NewSpecialTaskActivity extends BaseActivity implements DateTimeDialog.MyOnDateSetListener{
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_new_special_task);
        initView();
        initData();
    }
    ...
    private void initData() {
        ...
        mlvOpion.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
            ...
            public boolean onItemLongClick(AdapterView<?> parent, View view,final int position, long id) {
                AlertDialog.Builder tuichuDialog = new AlertDialog.Builder(NewSpecialTaskActivity.this)
                        .setMessage("删除此项？");
...
//                                ScreenUtils.setListViewHeightBasedOnChildren(getApplicationContext(),mlvFujian);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()){
            ...
            case R.id.linear_add:
                if(checkdata()){
                    //                     标题       内容      联系人     日期    隐患      隐患id   name
                    modlue.saveSpecialTask(taskName,taskContent,contactId,teakdate,troubleId,hiddenId,hiddenNum);
                }
                break;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(resultCode==RESULT_OK&&data!=null){
            if(requestCode==INT_TROUBLE_NAME_REQUEST){
...
//                        ScreenUtils.setListViewHeightBasedOnChildren(this,mlvFujian);
                ...
                if(path==null){
                    Toast.makeText(this,"此文件无效，请重新选择",Toast.LENGTH_SHORT).show();
                    return;
                }
...
//        }
    ...
    public void resultData(Message msg) {
        ...
        switch (msg.what){
            case TaskModlue.INT_SAVE_SPECIAL_TASK_SUCCESS:
                if(filepath!=null&&filepath.size()>0) {
                    String id = msg.obj.toString();
                    try {
                        FileUploadModule.uploadFiles(this,handler,
                                ConnectionURL.STR_SAVE_SPECIAL_TASK_UPLOAD+ MyApplication.getStringSharedPreferences("JSESSIONID", "")+ ConnectionURL.HTTP_SERVER_END
                                , filepath, id);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                ...
                break;
            case TaskModlue.INT_SAVE_SPECIAL_TASK_FILED:
                toLoginActivity(msg);
                break;
