 ...
 * 废弃
 */
public class STodoDeailyDetailActivity extends BaseActivity{
    ...
    private void initView() {
        ...
        LinearLayout mlinearContract=(LinearLayout)findViewById(R.id.linear_special_contract) ;

        mlinearContract.setVisibility(View.VISIBLE);
        mlinearMaintain.setVisibility(View.VISIBLE);
        mlinearTrouble.setVisibility(View.VISIBLE);
        mtvTitle.setText("日常任务详情");
        mtvSave.setText("");
    ...
    private void doApproveOrUpload(){
        ...
        if(task.getStatus()==3){
            intent1.setAction("android.intent.action.MAINTENANCEUPLOADACTIVITY");
...
//                break;
            case TodoModule.INT_GET_FORM_URL_SUCCESS:
                ...
                KLog.d(url);
                modlue.getDeilayDetail(sb.toString());
                break;
            case TodoModule.INT_GET_DEILYDEILY_SUCCESS:
                task=(Task)msg.obj;
                showData();
                break;
            ...
            tuichuDialog.setPositiveButton("下载",
                    new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            DownLoadImageTask task = new DownLoadImageTask(STodoDeailyDetailActivity.this,
                                    handler,
                                    filename);
                            StringBuffer sb = new StringBuffer();
            ...
            switch (v.getId()){
                ...
                case R.id.btn_jl_dialog_commit:
                    if(checkJLCommitData()){
                        modlue.commitDeliayMeterage(task.getTaskId(),task.getActtaskId(),task.getActprocInsId(),task.getActtaskDefKey(),"计量确认","yes",trouble.getTaskId(),jltroubleId,jltroubleContent);
                    }
                    break;
                case R.id.btn_jl_dialog_turndown:
                    modlue.commitDeliayMeterage(task.getTaskId(),task.getActtaskId(),task.getActprocInsId(),task.getActtaskDefKey(),"驳回","no",trouble.getTaskId(),null,null);
                    break;
